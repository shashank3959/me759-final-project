...
__global__ void GaussianNBCalcKernel(const double *d_data, const int *d_labels,
	double *f_stats_, int *class_count, unsigned int n_samples_, unsigned int n_classes_,
	unsigned int n_features_) {
		...
		if (feat_col < n_features_) { /* End condition check */
			...
			for (i = 0; i < n_samples_; ++i) {
				...
				f_stats_[RM_Index(row, feat_col, n_features_)] += d_data[RM_Index(i, feat_col, n_features_)];
...
__global__ void GaussianNBLearnKernel(double *feature_probs, double *f_stats_, int *class_count,
	...
	unsigned int n_features_) {
		...
		if (feat_col < n_features_) { /* End condition check */
			...
			for (i = 0; i < n_classes_; ++i) {
				...
				if (feat_col == 0) {
					feature_probs[i] = class_count[i]*1 / (double)(n_samples_);
...
__global__ void GaussianNBProbKernel(double *feature_probs, double *class_priors,
	const double *d_data, unsigned int n_samples_, unsigned int n_classes_,
	unsigned int n_features_) {
		...
		if (feat_col < n_features_) { /* End condition check */
			...
			for (i = 0; i < n_classes_; ++i) {
				...
				feature_probs[RM_Index(i, feat_col, n_features_)] += pow(lfm[RM_Index(i, feat_col, n_features_)]-f_stats_[RM_Index(row, feat_col, n_features_)],2);

				if (feat_col == 0) {
					//TODO: Change index of f_stats_ or introduce new variable for 1/(sqrt() term to avoid indexing issue 
					f_stats_[RM_Index(i, feat_col, n_features_)] /= class_count[i];
					f_stats_[RM_Index(i, feat_col, n_features_)] = 1.0 / sqrt(2 * M_PI * feature_probs[RM_Index(i, feat_col, n_features_)]);
		...
		if (sample_num < test_size) {
			for (i = 0; i < n_classes_; ++i) { /* For each class */
				...
				for (j=0; j < n_features_; ++j) { /* For each feature */
					// reference: p[lab] *= f_stats_[lab][2][i] * exp(-pow(vec[i] - f_stats_[lab][0][i], 2) / (2 * f_stats_[lab][1][i]));
...
void GaussianNB::train(vector<double> data, vector<int> labels) {
	...
	cudaMemcpy(d_label_list, thrust::raw_pointer_cast(thr_label_list.data()),n_classes_ * sizeof(int), cudaMemcpyHostToDevice);
	...
	GaussianNBCalcKernel<<<blocks_per_grid, threads_per_block>>>(d_data, d_labels,
		f_stats_, class_count, train_size, n_classes_, n_features_);
	...
	for (unsigned int i = 0; i < n_classes_; ++i) {
		thrust::device_vector<double> temp_vec(f_stats_ + (n_features_*i),f_stats_ + (n_features_*(i+1)));
	...
	GaussianNBLearnKernel<<<blocks_per_grid, threads_per_block>>>(f_stats_, feature_probs,class_count,
		lfm, train_size, n_classes_, n_features_);
	cudaDeviceSynchronize();

	// prob kernel

	GaussianNBProbKernel<<<blocks_per_grid, threads_per_block>>>(feature_probs,class_priors,
	*d_data,n_samples_,n_classes_,n_features_) 
...
int GaussianNB::predict(vector<double> data, vector<int>  labels) {
	...
	GaussianNBTestKernel<<<blocks_per_grid, threads_per_block>>>(d_data, d_labels,
...
__global__ void MultinomialNBCalcKernel(const double *d_data, const int *d_labels,
...
__global__ void MultinomialNBLearnKernel(double *feature_probs, double *class_priors,
	const double *d_row_sums, unsigned int n_samples_, unsigned int n_classes_,
	unsigned int n_features_) {
		...
		if (feat_col < n_features_) { /* End condition check */
			...
			for (i = 0; i < n_classes_; ++i) {
				// TODO: Add Laplace Smoothing
				feature_probs[RM_Index(i, feat_col, n_features_)] =log (feature_probs[RM_Index(i, feat_col, n_features_)] / d_row_sums[i]);
...
__global__ void MultinomialNBTestKernel(const double *d_data, const int *d_labels,
	const double *feature_probs, const double *class_priors, int test_size,
	int n_classes_, int n_features_, int *score) {
		/* Each thread will take one term */
		unsigned int tidx = threadIdx.x;
		...
		if (sample_num < test_size) {
			for (i = 0; i < n_classes_; ++i) { /* For each class */
				...
				for (j=0; j < n_features_; ++j) { /* For each feature */
					prob_class += (d_data[RM_Index(sample_num, j, n_features_)] *feature_probs[RM_Index(i, j, n_features_)]);
				}
...
void MultinomialNB::train(vector<double> data, vector<int> labels) {
	...
	dim3 threads_per_block(THREADS_PER_BLOCK);
	dim3 blocks_per_grid(ceil(float(n_features_) / float(threads_per_block.x)));
	MultinomialNBCalcKernel<<<blocks_per_grid, threads_per_block>>>(d_data, d_labels,
	...
	/* Find total number of terms in each class */
	for (unsigned int i = 0; i < n_classes_; ++i) {
		thrust::device_vector<double> temp_vec(feature_probs + (n_features_*i),feature_probs + (n_features_*(i+1)));
		d_row_sums[i] = thrust::reduce(thrust::device, temp_vec.begin(), temp_vec.end());
		}

	thrust::device_vector<double> temp_vec(d_row_sums, (d_row_sums + n_classes_));

	MultinomialNBLearnKernel<<<blocks_per_grid, threads_per_block>>>(feature_probs, class_priors,
...
int MultinomialNB::predict(vector<double> data, vector<int>  labels) {
	...
	dim3 threads_per_block(THREADS_PER_BLOCK);
	dim3 blocks_per_grid(ceil(float(test_size) / float(threads_per_block.x)));
	MultinomialNBTestKernel<<<blocks_per_grid, threads_per_block>>>(d_data, d_labels,
	feature_probs, class_priors, test_size, n_classes_, n_features_, score);
...
__global__ void BernoulliNBCalcKernel(const double *d_data, const int *d_labels,
	double *feature_probs, double *class_count_, unsigned int n_samples_, unsigned int n_classes_,
	unsigned int n_features_) {
...
// Kernel divides each row by a number and takes log 
__global__ void BernoulliNBLearnKernel(double *feature_probs, double *class_count_,
	const double *d_row_sums, unsigned int n_samples_, unsigned int n_classes_,
	unsigned int n_features_) {
		...
		if (feat_col < n_features_) { // End condition check 
			// For each label 
			for (i = 0; i < n_classes_; ++i) {
				feature_probs[RM_Index(i, feat_col, n_features_)] /= class_count_[i]; // d_row_sums[i];
...
__global__ void BernoulliNBTestKernel(const double *d_data, const int *d_labels,
	const double *feature_probs, const double *class_count_, int test_size,
	int n_classes_, int n_features_, int *score) {
		// Each thread will take one term 
		unsigned int tidx = threadIdx.x;
		unsigned int sample_num = tidx + (blockIdx.x * blockDim.x);
		...
		if (sample_num < test_size) {
			for (i = 0; i < n_classes_; ++i) { // For each class 
				...
				for (j=0; j < n_features_; ++j) { // For each feature 
					prob_class *= pow(feature_probs[RM_Index(i, j, n_features_)], d_data[RM_Index(sample_num, j, n_features_)]);
					prob_class *= pow((1-feature_probs[RM_Index(i, j, n_features_)]),(1-d_data[RM_Index(sample_num, j, n_features_)]));
...
void BernoulliNB::train(vector<double> data, vector<int> labels) {
	...
	double *d_data;
	cudaMallocManaged(&d_data, (n_features_ * train_size) * sizeof(double));
	cudaMemcpy(d_data, &data[0], (n_features_ * train_size) * sizeof(double),cudaMemcpyHostToDevice);

	// NOTE: Memory Operation, put checks later
	int *d_labels;
	cudaMallocManaged(&d_labels, train_size * sizeof(int));
	cudaMemcpy(d_labels, &labels[0], train_size * sizeof(int),cudaMemcpyHostToDevice);
	...
	dim3 threads_per_block(THREADS_PER_BLOCK);
	dim3 blocks_per_grid(ceil(float(n_features_) / float(threads_per_block.x)));
	BernoulliNBCalcKernel<<<blocks_per_grid, threads_per_block>>>(d_data, d_labels,
		feature_probs, class_count_, train_size, n_classes_, n_features_);
	cudaDeviceSynchronize();
	...
	thrust::device_vector<double> temp_vec(d_row_sums, (d_row_sums + n_classes_));
	cout<<"class_count_ 0 "<<class_count_[0]<<" class_count_ 1 "<<class_count_[1]<<endl;
	BernoulliNBLearnKernel<<<blocks_per_grid, threads_per_block>>>(feature_probs, class_count_,
		d_row_sums, train_size, n_classes_, n_features_);
	cudaDeviceSynchronize();
...
int BernoulliNB::predict(vector<double> data, vector<int>  labels) {
	...
	dim3 blocks_per_grid(ceil(float(test_size) / float(threads_per_block.x)));
	MultinomialNBTestKernel<<<blocks_per_grid, threads_per_block>>>(d_data, d_labels,
	feature_probs, class_count_, test_size, n_classes_, n_features_, score);
	cudaDeviceSynchronize();
...
__global__ void ComplementNBCalcKernel(const double *d_data, const int *d_labels,double *feature_probs, double *class_count_, unsigned int n_samples_, unsigned int n_classes_,unsigned int n_features_,int  *all_occur_per_term) {
		...
		if (feat_col < n_features_) { // End condition check 
			...
			for (i = 0; i < n_samples_; ++i) {
				...
				// No race condition since each thread deals with one feature only
				feature_probs[RM_Index(row, feat_col, n_features_)] += d_data[RM_Index(i, feat_col, n_features_)];
				all_occur_per_term[feat_col] +=d_data[RM_Index(i, feat_col, n_features_)];
				// WARNING: thread divergence :(
				if (feat_col == 0) {
					class_count_[row] += 1;
				}
...
// Kernel divides each row by a number and takes log 
__global__ void ComplementNBLearnKernel(double *feature_probs, double *d_row_sums, double *class_count_,int  *all_occur_per_term, double all_occur, unsigned int n_samples_, unsigned int n_classes_,unsigned int n_features_) {
		...
		if (feat_col < n_classes_ ) { // for each class 
			...
			for (i = 0; i < n_features_; ++i) { // For each feature 
				...
				num_sum = all_occur_per_term[i] - feature_probs[RM_Index(i, feat_col, n_features_)];
				feature_probs[RM_Index(i, feat_col, n_features_)] = log(((double)num_sum + 1) /((double)den_sum + 1));
...
__global__ void ComplementNBNormalizeKernel(double *feature_probs, double *d_row_sums,unsigned int n_classes_,unsigned int n_features_) {
...
__global__ void ComplementNBTestKernel(const double *d_data, const int *d_labels,const double *feature_probs, const double *class_count_, int test_size,int n_classes_, int n_features_, int *score) {
		// Each thread will take one term 
		unsigned int tidx = threadIdx.x;
		unsigned int sample_num = tidx + (blockIdx.x * blockDim.x);
		unsigned int i = 0, j = 0;
...
void ComplementNB::train(vector<double> data, vector<int> labels) {
	...
	/* Other initializations */
	//cudaMallocManaged(&feature_frequencies_, (n_classes_ * n_features_) * sizeof(double));
	cudaMallocManaged(&feature_probs, (n_classes_ * n_features_) * sizeof(double));
	...
	dim3 threads_per_block(THREADS_PER_BLOCK);
	dim3 blocks_per_grid(ceil(float(n_features_) / float(threads_per_block.x)));
	ComplementNBCalcKernel<<<blocks_per_grid, threads_per_block>>>(d_data, d_labels,feature_probs, class_priors, train_size, n_classes_, n_features_,all_occur_per_term);
	cudaDeviceSynchronize();
	...
	thrust::device_vector<double> temp_vec(d_row_sums, (d_row_sums + n_classes_));
	double all_occur;
	all_occur = (double) accumulate(all_occur_per_term, all_occur_per_term + n_features_ , 0);
	cout<<" all occur "<<all_occur<<endl;

	cout<<"n_classes_ "<<n_classes_<<"fb sdfhjdfjs "<<endl;
	ComplementNBLearnKernel<<<blocks_per_grid, threads_per_block>>>(feature_probs,d_row_sums,class_priors,all_occur_per_term, all_occur, train_size, n_classes_, n_features_);
	cudaDeviceSynchronize();

	//normalize weights 

	ComplementNBNormalizeKernel<<<blocks_per_grid, threads_per_block>>>(feature_probs,d_row_sums,n_classes_,n_features_);
	cudaDeviceSynchronize();
...
int ComplementNB::predict(vector<double> data, vector<int>  labels) {
	...
	ComplementNBTestKernel<<<blocks_per_grid, threads_per_block>>>(d_data, d_labels,
	feature_probs, class_priors, test_size, n_classes_, n_features_, score);
