...
void GaussianNB::train(vector<vector<double>> data, vector<int> labels) {
  ...
  int j = 0;
// init mean and var
#pragma omp parallel private(lab, i, j) num_threads(1)
  {
    //#pragma omp section
    //{
#pragma omp for
...
#pragma omp for
    ...
    for (lab = 0; lab < labels_list_.size(); lab++) {
      for (j = 0; j < features_count_; j++) {
        ...
        f_stats_[lab][2][j] =
            1.0 / sqrt(2 * M_PI * f_stats_[lab][1][j]); // 1/sqrt(2*PI*var)
      }
    ...
    for (k = 0; k < X_test.size(); k++) {
      ...
      for (lab = 0; lab < labels_list_.size(); lab++) {
        ...
        for (i = 0; i < features_count_; i++) {
          p[lab] *=
              f_stats_[lab][2][i] * exp(-pow(vec[i] - f_stats_[lab][0][i], 2) /
                                        (2 * f_stats_[lab][1][i]));
        }
        ...
        for (auto i = 0; i < train.size(); i++)
        {
                ...
                for (auto l = 1; l < train[0].size(); l++)
                {
                        ...
                        if (sum_x.find(labels) == sum_x.end()) {
                                vector<double> empty;
                                for (unsigned int j = 1; j < train[0].size();
j++) { empty.push_back(0.0);
                                }
                                sum_x[labels] = empty;
                        }
...
it->first<<priors[it->first];
                ...
                for (unsigned int i = 0; i < it->second.size(); i++) {
                        feature_means.push_back(sum_x[it->first][i] /
n[it->first]);
                }
                ...
                for (unsigned int i = 0; i < data[it->first].size(); i++) {
                        for (unsigned int j = 0; j < data[it->first][i].size();
j++) { feature_variances[j] += (data[it->first][i][j] - feature_means[j]) *
(data[it->first][i][j] - feature_means[j]);
                        }
                }
                ...
                for (unsigned int i = 0; i < feature_means.size(); i++) {
                        double mnl = (sum_x[it->first][i] + alpha) /
(multinomial_sums[it->first] + (alpha * feature_means.size()));
                        //cout << sum_x[it->first][i] << " + 1 / " <<
multinomial_sums[it->first] << " + " << feature_means.size() << endl;
                        multinomial_likelihoods[it->first].push_back(mnl);
                }

                for (unsigned int i = 0; i < feature_means.size(); i++) {
                        printf("%i\t%2.3f\t%2.3f\t%2.3f\t%2.3f\n", i + 1,
feature_means[i], feature_variances[i], sqrt(feature_variances[i]),
multinomial_likelihoods[it->first][i]);
                        //cout << feature_means[i] << "\t" <<
sqrt(feature_variances[i]) << endl;
                }
        ...
        for (auto i = 0; i < X_test.size(); i++)
        {
                ...
                for (auto it = priors.begin(); it != priors.end(); it++) {
                        ...
                        for (auto j = 0; j < features_count_; j++) {
                                numer *=
pow(multinomial_likelihoods[it->first][j], vec[j]);
                        }
